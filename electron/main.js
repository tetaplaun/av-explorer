...
ipcMain.handle("get-drives", async () => {
  if (process.platform === "win32") {
    ...
    try {
      const psCmd =
        "powershell -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command \"Get-PSDrive -PSProvider FileSystem | Select-Object Name, @{Name='FreeSpace';Expression={$_.Free}}, @{Name='TotalSize';Expression={$_.Used + $_.Free}} | ConvertTo-Json\""
...
async function getEncodedDate(filePath) {
  try {
    ...
    const { stdout } = await execFilePromise(
      "mediainfo",
      ["--Output=General;%Encoded_Date%", filePath],
      {
        timeout: 3000,
        windowsHide: true,
      }
...
// Get encoded dates for a single file or small batch (progressive loading)
ipcMain.handle("get-encoded-dates-progressive", async (event, fileItems) => {
  const results = {}

  // Process files one by one for progressive loading
